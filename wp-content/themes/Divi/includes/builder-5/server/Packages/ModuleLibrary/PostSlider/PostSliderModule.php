...
class PostSliderModule implements DependencyInterface {
	...
	/**
	 * Expand inherited text-color rows so style statements can print the matching media queries.
	 *
	 * The style renderer only iterates breakpoints that already exist in the attr array. When a
	 * larger breakpoint such as `ultraWide` inherits its text color from `widescreen`, we still
	 * need a synthetic `value.color` row so overlay and text-shadow declarations emit the matching
	 * CSS instead of falling back to the desktop base rule.
	 *
	 * @since ??
	 *
	 * @param array $attr Text color attributes.
	 *
	 * @return array
	 */
	private static function get_responsive_text_color_style_attr( array $attr ): array {
		if ( empty( $attr ) ) {
			return [];
		}
		...
		$base_value          = array_key_exists( 'value', $attr[ $base_breakpoint ] ?? [] )
			? $attr[ $base_breakpoint ]['value']
		...
		$responsive_attr     = $attr;

		foreach ( $output_breakpoints as $breakpoint ) {
			$explicit_value_color = $attr[ $breakpoint ]['value']['color'] ?? null;
			...
			$should_render_value  = 'desktop' !== $breakpoint
				...
				&& ! in_array( $explicit_value_color, $valid_colors, true );

			if ( $should_render_value ) {
				$responsive_attr[ $breakpoint ] = $responsive_attr[ $breakpoint ] ?? [];
				$responsive_attr[ $breakpoint ]['value'] = array_merge(
					$responsive_attr[ $breakpoint ]['value'] ?? [],
					[
	...
	public static function slide_overlay_tint_style_declaration( array $params, array $attrs ): string {
		...
		$explicit_overlay_background   = $attrs['slideOverlay']['decoration']['background'][ $breakpoint ]['value'] ?? null;
		$custom_overlay_background     = null !== $explicit_overlay_background
			? $explicit_overlay_background
			: ModuleUtils::use_attr_value(
				[
					'attr'         => $attrs['slideOverlay']['decoration']['background'] ?? [],
					'breakpoint'   => $breakpoint,
					'state'        => 'value',
					'mode'         => 'getOrInheritClosest',
					'defaultValue' => [],
		...
		$custom_overlay_color      = $custom_overlay_background['color'] ?? '';

		if ( ! empty( $custom_overlay_color ) ) {
			return $style_declarations->value();
		}

		$explicit_text_attr_value = $attrs['module']['advanced']['text']['text'][ $breakpoint ]['value'] ?? null;
		$text_attr_value          = null !== $explicit_text_attr_value
			? $explicit_text_attr_value
			: ModuleUtils::use_attr_value(
				[
		...
		$text_color     = $text_attr_value['color'] ?? '';
	...
	public static function module_styles( $args ) {
		...
		$responsive_text_color_attr  = self::get_responsive_text_color_style_attr( $attrs['module']['advanced']['text']['text'] ?? [] );
								...
								'advancedStyles'           => [
									...
									[
										...
										'props'         => [
											...
											'attr'                => $responsive_text_color_attr,
