...
class BlockParserStore {
	...
	private static function _parse_blocks_with_error_recovery( string $content ): array {
		// Parse with WordPress core parser directly to avoid BlockParserStore/orderIndex side effects.
		$core_parser = new \WP_Block_Parser();
		$blocks      = self::_filter_whitespace_blocks( $core_parser->parse( $content ) );
