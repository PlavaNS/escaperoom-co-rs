...
Treat missing or thin coverage as a finding when behavior changes, bugfixes, or new API paths are introduced. If no tests are added or updated, require a justification or a clear manual validation plan. Favor broader regression coverage that would catch related failures, not just a single narrow case. Do not suggest test framework preferences, refactors, or general improvements.
...
- When a PR fixes a bug, require a regression test that would have caught it and similar related failures (cover adjacent scenarios, not just the exact repro).
